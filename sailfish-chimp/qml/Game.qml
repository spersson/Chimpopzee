...
Page {
	...
	onActiveChanged: {
		if(active) {
			// on harmattan this is needed since setting it to true after being deactivated
			// has no effect, needs to be done after activated. We know that we are not in Running state.
			gPlatform.allowSwipe = true;
		} else if(state === "Running") {
			state = "Pause Menu";
		}
	}

	onStateChanged: gPlatform.allowSwipe = state != "Running"
